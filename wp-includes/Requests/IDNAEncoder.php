...
class Requests_IDNAEncoder {
	...
	const uikit_TMAX         = 26;
	const uikit_SKEW         = 38;
	const uikit_DAMP         = 700;
	const uikit_INITIAL_BIAS = 72;
	const uikit_INITIAL_N    = 128;
	/**#@-*/
	...
	public static function punycode_encode($input) {
		$output = '';
#		let n = initial_n
		$n = self::uikit_INITIAL_N;
#		let delta = 0
		$delta = 0;
#		let bias = initial_bias
		$bias = self::uikit_INITIAL_BIAS;
...
#					for k = base to infinity in steps of base do begin
					for ($k = self::uikit_BASE; ; $k += self::uikit_BASE) {
#						let t = tmin if k <= bias {+ tmin}, or
#								tmax if k >= bias + tmax, or k - bias otherwise
						if ($k <= ($bias + self::uikit_TMIN)) {
							$t = self::uikit_TMIN;
						}
						elseif ($k >= ($bias + self::uikit_TMAX)) {
							$t = self::uikit_TMAX;
						}
						else {
							$t = $k - $bias;
...
#						output the code point for digit t + ((q - t) mod (base - t))
						$digit = $t + (($q - $t) % (self::uikit_BASE - $t));
						$output .= self::digit_to_char($digit);
#						let q = (q - t) div (base - t)
						$q = floor(($q - $t) / (self::uikit_BASE - $t));
#					end
					}
...
#		if firsttime then let delta = delta div damp
		if ($firsttime) {
			$delta = floor($delta / self::uikit_DAMP);
		}
...
#		while delta > ((base - tmin) * tmax) div 2 do begin
		$max = floor(((self::uikit_BASE - self::uikit_TMIN) * self::uikit_TMAX) / 2);
		while ($delta > $max) {
#			let delta = delta div (base - tmin)
			$delta = floor($delta / (self::uikit_BASE - self::uikit_TMIN));
#			let k = k + base
			$k += self::uikit_BASE;
#		end
		}
#		return k + (((base - tmin + 1) * delta) div (delta + skew))
		return $k + floor(((self::uikit_BASE - self::uikit_TMIN + 1) * $delta) / ($delta + self::uikit_SKEW));
	}
}