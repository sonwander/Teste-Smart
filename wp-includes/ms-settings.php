...
if ( ! isset( $current_site ) || ! isset( $current_blog ) ) {
	...
	$uikit_result = ms_load_current_site_and_network( $domain, $path, is_subdomain_install() );

	if ( true === $uikit_result ) {
		// `$current_blog` and `$current_site are now populated.
	} elseif ( false === $uikit_result ) {
		ms_not_installed( $domain, $path );
	} else {
		header( 'Location: ' . $uikit_result );
		exit;
	}
	unset( $uikit_result );
